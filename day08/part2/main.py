...
def find_points_on_line_with_distance(starting_point, i_move, j_move, max_i, max_j):
    points = []
    i = starting_point[0]
    j = starting_point[1]
    ...
    current_i_move = i_move
    current_j_move = j_move
    while (
        i + current_i_move >= 0
        and i + current_i_move < max_i
        and j + current_j_move >= 0
        and j + current_j_move < max_j
    ):
        points.append((i + current_i_move, j + current_j_move))
        current_i_move += i_move
        current_j_move += j_move
...
def find_antinodes_for_pair(max_i, max_j, antenna_a, antenna_b, anti_nodes):
    i_move, j_move = get_directional_distance_move(antenna_a, antenna_b)

    print(f"Move: i={i_move}, j={j_move}")
    points_a = find_points_on_line_with_distance(antenna_a, i_move, j_move, max_i, max_j)
    points_b = find_points_on_line_with_distance(antenna_b, i_move, j_move, max_i, max_j)
    points = points_a + points_b
    for p in points:
        if p[0] < 0 or p[1] < 0 or p[0] >= max_i or p[1] >= max_j:
            print(
                "Out of bounds, antenna_a: ",
                antenna_a,
                " antenna_b: ",
                antenna_b,
                " point: ",
